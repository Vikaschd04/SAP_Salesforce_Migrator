...
# Directories a build tool owns. Anything under one of these was written by a
# machine from a source of truth that still exists — items.xml, a WSDL, a proto.
_GENERATED_DIRS = {"gensrc", "generated-sources", "generated-src", "generated"}

# The banner Hybris (and most code generators) stamp on their own output.
_GENERATED_BANNER = re.compile(
    r"THIS FILE IS GENERATED|WILL BE OVERWRITTEN|DO NOT (?:EDIT|MODIFY) THIS FILE|"
    r"@generated\b|Generated by the platform build|Auto-generated by",
    re.IGNORECASE,
)


def _is_generated_source(path: str, source: str) -> str:
    """Was this file written by a build tool rather than a person?

    Returns the reason it is generated, or "" for hand-written code.

    This is the single largest cost lever in a real migration. A Hybris estate's
    `bootstrap/gensrc` holds one `*Model` class per item type — thousands of files,
    none of them containing a decision anybody made. Converting them is not merely
    waste: the target's own build regenerates the equivalent from the migrated data
    model, so the output would be thrown away even if it were perfect.

    **Detection is deliberately strict, and only ever uses machine-written evidence:**
    a build-owned directory, or a generator's own banner. The two errors here are not
    symmetric — converting a generated file costs money, while skipping a hand-written
    one loses business logic — so the rule under-skips by design.

    In particular, `extends Generated*` is NOT evidence. Hybris generates an abstract
    half and a developer-editable half of the same pair: `AcmecoreConstants extends
    GeneratedAcmecoreConstants` is where a team puts their real configuration keys.
    A rule keyed on that name would delete exactly the hand-written half.
    """
    parts = {seg.lower() for seg in Path(path).parts}
    hit = parts & _GENERATED_DIRS
    if hit:
        return f"lives under {sorted(hit)[0]}/, a build-generated directory"

    # Only the file's own header counts — a banner quoted 400 lines into a string
    # literal is not the file declaring itself generated.
    banner = _GENERATED_BANNER.search(source[:2000])
    if banner:
        return f"carries a generator banner ({banner.group(0).strip()!r})"
    return ""
...
def ingest(input_dir: str) -> dict:
    ...
    classes, test_classes, unreadable, generated = [], [], [], []
    for java_file in sorted(java_files, key=lambda p: p.name):
        # Checked before parsing: a generated file costs nothing to identify from its
        # path, and there is no point spending a parse on something we will not migrate.
        try:
            head = java_file.read_text(encoding="utf-8", errors="replace")[:2000]
        except OSError:
            head = ""
        why = _is_generated_source(str(java_file), head)
        if why:
            generated.append({"class_name": java_file.stem, "file": str(java_file),
                              "reason": why})
            continue
    ...
    if generated:
        print(f"  · {len(generated)} build-generated file(s) held aside "
              f"(not migrated — the target regenerates these from the data model; "
              f"listed in the completeness ledger)")
