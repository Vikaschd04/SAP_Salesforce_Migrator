...
from src.adapters.apex_characterization import literal
...
def test_mines_recorded_values_not_opinions():
    ...
    # Mining records the fact — a decimal, verbatim source, canonical value. What that
    # literal *looks like* is the target's business, so it is asserted separately.
    assert b["args"][0] == {"source": 'new BigDecimal("200.00")', "kind": "decimal",
                            "value": "200.00"}
    assert b["expected"]["value"] == "180.00"
    assert literal(b["args"][0]) == "200.00"     # new BigDecimal("200.00") → Apex Decimal
    assert literal({"kind": "enum", "value": "GOLD"}) == "'GOLD'"   # Apex enums cross as strings
    assert literal({"kind": "object", "value": None}) is None       # an object graph is no literal
...
def test_summary_and_headline_are_honest():
    ...
    assert "No tests found in the source" in headline(summarise([]))
