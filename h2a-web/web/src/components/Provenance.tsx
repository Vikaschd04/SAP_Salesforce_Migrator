...
export interface ProvenanceData {
  artifacts: {
    ...
    links: { target: string; target_lines: number[]; source: string; source_lines: number[];
             source_class: string; basis: string; confidence: string }[];
    target_without_origin: { target: string; target_lines: number[] }[];
    source_without_target: { source: string; source_class: string; source_lines: number[] }[];
  }[];
  summary: { artifacts: number; linked: number; methods: number; coverage: number | null;
             target_without_origin: number; source_without_target: number };
}
...
export default function Provenance({ p }: { p: ProvenanceData | null }) {
  ...
  return (
    <div className="tabpanel">
      ...
      {p.summary.source_without_target > 0 && (
        <div className="rule-warn">
          <strong>{p.summary.source_without_target} Java method(s) have no Apex counterpart.</strong>{' '}
      ...
      {p.artifacts.map((m) => (
        <div key={m.target} className="pv-art">
          <button className="pv-head" onClick={() => setOpen(open === m.target ? null : m.target)}>
            ...
            {m.source_without_target.length > 0 &&
              <span className="pv-lost">{m.source_without_target.length} unmatched</span>}
          </button>
          {open === m.target && (
            <div className="pv-body">
              {m.links.length > 0 && (
                <table className="rule-tbl">
                  ...
                  <tbody>
                    {m.links.map((l, i) => (
                      <tr key={i}>
                        <td><code>{l.target}</code><span className="rule-id">lines {l.target_lines[0]}–{l.target_lines[1]}</span></td>
                        <td className="faint">←</td>
                        <td><code>{l.source_class}.{l.source}</code><span className="rule-id">lines {l.source_lines[0]}–{l.source_lines[1]}</span></td>
              ...
              {m.target_without_origin.length > 0 && (
                <div className="pv-resid">
                  ...
                  {m.target_without_origin.map((o) => (
                    <code key={o.target} className="pv-chip">{o.target}</code>
                  ))}
                </div>
              )}
              {m.source_without_target.length > 0 && (
                <div className="pv-resid bad">
                  ...
                  {m.source_without_target.map((u) => (
                    <code key={u.source} className="pv-chip">{u.source_class}.{u.source}</code>
                  ))}
                </div>
              )}
            </div>
