...
| A5 | Null-returning getters | `getAttribute()` returns null freely in Hybris. Apex arithmetic on a null `Decimal` throws at *runtime*, not compile time, so it passes deploy verification and fails in the org. | **covered** — 1.30. Asks a narrower question the AST answers exactly: *is the result dereferenced in a method that never null-checks it?* On the reference corpus that is 3 findings and **zero** false positives across four real guards — ternary, short-circuit, `!= null`, and a check placed after the use |
| A6 | Many-to-many relations | Become junction objects — but Salesforce allows a maximum of **2 master-detail** relationships per object. | **moot, and stated rather than closed** — checked in 1.20b: the emitter never produces a master-detail relationship at all. Every one-to-many becomes a Lookup, so the 2-MD ceiling cannot be reached. That is the safe default (a Lookup can be promoted to master-detail; the reverse constrains data loading), but it is a *silent* default — nothing records that cascade-delete and roll-up semantics were not carried across. Recorded as its own item rather than counted as covered. |
| A7 | 500-custom-field ceiling | Hybris `Product` in a real estate routinely exceeds Salesforce's per-object field limit. | **covered** — 1.20b. Reported, never trimmed: which attributes to drop, merge or move is a modelling decision, and an automatic answer would silently discard whichever sorted last. Leads the sign-off, because a deploy that cannot land is a blocker rather than a caveat. |
...
| B6 | Savepoint semantics | Two shapes, not one. Rollback-by-throwing needs no savepoint — Apex rolls the whole request back on an uncaught exception, and each `setSavepoint()` costs one of the 150 DML statements. Rollback-by-catching is a partial rollback and a savepoint is its only expression; note a rollback does *not* restore governor counters. | **covered** — `TRANSACTION_SHAPE` classifies the shape and grounds the Builder; the contradictory `TRANSACTIONAL` rule was removed — 1.21c |
