...
| A6 | Relations | Cardinality alone does not decide the mapping. A `partof` end is a composition and only Master-Detail cascades; many-to-many needs a junction object; one-to-one has no target type at all; and an end that is an out-of-the-box type has no `__c` object to point at. | **covered** — `src/relations.py` decides all four shapes, demotes past the 2-master-detail ceiling with the reason recorded, and refuses to invent an object for a standard type. Every relation and its cost is in `MAPPING.md`; the unconvertible ones are sign-off caveats — 1.32 |
