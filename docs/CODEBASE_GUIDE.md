...
│   ├── processes.py       ← business processes: parsed, resolved, reported
│   ├── flow_generator.py  ← that state machine → a Salesforce Flow (deterministic)
│   │
...
| [flow_generator.py](../h2a-mvp/src/flow_generator.py) | Translates a parsed process into Flow metadata + `@InvocableMethod` wrappers. Deterministic — the state machine is fully specified in the source, so nothing about your process shape can be hallucinated. Ships **Draft**, and reports what it inferred. |
