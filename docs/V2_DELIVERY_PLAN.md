...
| ~~1.45~~ | ✅ **Four more real modules, six more defects** | — | Ran `magefan/module-blog` (289 PHP), `Ethan3600/magento2-CronjobManager`, `bitExpert/magento2-force-login` and `markshust/…disabletwofactorauth`. Six defects, every one a *name resolving to something that does not exist*: a rename map including `manual` targets so a PHP type mapped to itself; a class in two targets taking whichever came last, naming a DAO the data model never produced; `Collection` resolving to `java.util.Collection`; `endswith("Model")` matching Magento's `AbstractModel`; a decorator implementing an interface derived from unit names rather than emitted ones; and `TODO_Event`, a placeholder that turned a finding into a compile error. **All four now compile clean: 294, 96, 53 and 2 files.** |
| ~~1.44~~ | ✅ **The cockpit speaks the running migration's language, and a field written nowhere** | — | The cockpit offered "Download SFDX package" for every run and its file browser only looked in `force-app`, so an Adobe→Hybris run showed **no generated files at all**. Labels now come from the output layout, which cannot disagree with what is on disk. Plus `UNKNOWN_ENTITY_FIELD`: a key written with `setData` that no table declares. Found `country_id` and `region` in a published module's GraphQL resolver, against a table declaring `country` and `state` — two fields of every appointment booked through the API, silently dropped at the insert. |
| ~~1.43~~ | ✅ **Teach the tool what the hand-migration knew** — `adapters/magento_modelling.py` | — | `db_schema.xml` declares foreign keys and the reader dropped them, so `customer_id` reached the target as an integer and the relationship was gone — the same oversight as `partof` in 1.32, one platform over. Declared keys now become real type references in items.xml. Everything the source only *hints* at — a `*_id` with no constraint, free text holding a closed set, an address across five columns, a table with no business key — is reported and grounds the Builder, never converted. On the Appointment module that is the same seven calls I made by hand. |
| ~~1.42~~ | ✅ **Hand-migrate a module, and fix what that exposes** | — | Migrated `mage-monk/Appointment` by hand to have a reference target, then compiled it against the stand-in: **16 errors, every one caused by the stand-in rather than the code.** Generated models were emitted as empty classes, `<enumtype>` was ignored entirely, attribute types like `Customer` had no stub, and `setResultClassList` was missing from the API surface. None of it showed while the tool's own output was stub bodies that never touch an accessor — so the `typechecked` rung would have collapsed the moment Phase 0 produced real logic. |
| ~~1.41~~ | ✅ **Backoffice config for admin UI** — `adapters/hybris_backoffice.py` | — | Magento's admin grids are declarative XML and so is Hybris Backoffice, so "no equivalent" was wrong about the larger half. The list view, editor area and advanced search for every item type are now generated from the items.xml this migration writes, with `requires-extension backoffice` and the module meta that make the file actually load. Custom buttons and column actions stay manual — they are widgets with behaviour — and every manual row now carries the planner's own reason instead of a generic "no emitter yet". Also split `AdminUi` from `View`: admin classes were being told that Spartacus composes pages from CMS components. |
| ~~1.40~~ | ✅ **Run against real third-party code** — `mage-monk/Appointment` | — | A public Magento 2 module, 24 PHP files, cloned and migrated. Six defects a hand-written fixture cannot produce: Magento framework types (`Redirect`, `RequestInterface`) emitted verbatim into Java signatures; triage calling 18 artifacts "routine" over 28 unresolvable types, because the check runs after the Builder finishes; three unrelated controllers all named `Index` folded into one service; a DAO claimed as converted with no file behind it; `emitted_as` keyed by a class name that is not unique; and the filename-vs-classname split reappearing the moment the planner qualified a name. Now 28 files, rung `typechecked`, 0 issues, and every `converted` row points at a file that exists. |
| ~~4.6~~ | ✅ **Docs and decks describe the product that exists** | — | The front door opened "SAP Hybris → Salesforce Apex Migrator" and the quick start showed one pipeline. README, HOW_TO_USE, HOW_IT_WORKS, COCKPIT_GUIDE, DEMO_SCRIPT, DIFFERENTIATORS, PRD/TRD/TDD/APP_FLOWS, ROADMAP and CODEBASE_GUIDE now state two pipelines and *why their strongest claims differ*. All three decks re-branded and regenerated. Deliberately not renamed: `h2a-mvp/` and `H2A_*` are identifiers, not branding. |
| ~~1.39~~ | ✅ **Reports describe the migration that ran** | — | The hazard report was never written for Adobe→Hybris at all: the orchestrator asked the *Hybris* scanner, found nothing in PHP, and skipped it — while the Adobe reader had already found 12 hazards and put them in the IR. Now `SourceAdapter.hazards()`. And the prose: the feasibility report opened "SAP Hybris to Salesforce Apex Migration" whichever pipeline produced it, listed `.java` sources beside `.cls` targets for a PHP→Java run, labelled Hybris services "Selector Pattern", and gave reviewers a checklist about governor limits and SOQL for a platform with neither. |
| ~~1.38~~ | ✅ **An industry-scale Adobe corpus, and what it found** — `Testing/acme-commerce-magento-full` | — | 48 files, 3 modules, 31 PHP: repositories, resource models, collections, a state machine, a console command, blocks and templates, webapi/acl/extension_attributes, PHPUnit suites. Migrating it emitted 32 Java files and the new compile rung rejected 12: a service whose *filename and class name disagreed* (two derivations of one name), and signatures naming PHP types that this migration itself renames. Also fixed: `identify` printed "Migration not implemented yet" above a list saying the pipeline was ready, and the build ran in one wavefront because the dependency graph is Java-only — the Adobe reader's own topological order was being discarded. Now 2 wavefronts, 32 files, rung `typechecked`, 0 issues. |
| ~~1.36~~ | ✅ **Provenance across a language boundary** | — | The headline read "traced to their **Java** origin" for every pipeline, so an Adobe→Hybris run reported methods traced to the language it had just *written*; same bug one line into `signoff`. Both now name the source language. The purity ratchet's vocabulary omitted `java`, which is why 4.4 parameterised the sentences either side and missed these — adding it caught the second one immediately. Also: a stub run's coverage measures placeholders, not a migration, and the report now says so instead of reading like a tool that invents 92% of its output. Verified end to end on real PHP→Java: 3 exact-name links and an invented method correctly isolated. |
...
| ~~3.10b~~ | ✅ **Real `javac` against a stand-in** — `adapters/hybris_stubs.py`, rung `typechecked` | No licence | ~30 hand-declared `de.hybris.platform.*` types plus model classes derived from the `items.xml` this tool writes. Found on its first run that every generated DAO referenced `<Item>Model` and imported none of them — 8 errors the static rung could not see, because it resolves a *name* and the name was known. Ranks below `compiled` deliberately: where a stub is wrong the compiler accepts wrong code just as confidently, and the sign-off says so. |
