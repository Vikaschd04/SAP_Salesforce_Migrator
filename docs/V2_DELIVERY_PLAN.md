...
| 1.31 | **MAPPING.md must report what was emitted** | — | Found while building 1.20a. The report derives names and types independently of the schema, so it disagrees with the metadata beside it. |
...
| 1.21b | **Savepoint mapping and re-entry guards** | B4, B6 | The original 1.21. Still worth doing, but it is about code the engine does not yet emit — no triggers are generated at all today, which is what 1.21a exposed. |
