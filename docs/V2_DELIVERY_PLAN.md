...
| 1.30 | **Null-safety pass** — getters that can return null, feeding arithmetic | A5 | Split out of 1.17 on purpose: it needs type resolution across files, and a regex version would be noisy in both directions. A noisy rule gets ignored, which is worse than no rule. |
...
| 1.23b | **Put the derived SOQL in front of the Builder and the customer** | — | `analyse()` produces checkable SOQL today but only the *blocking* cases surface. Feeding the derived query into generation, and listing it in a report, is the half that turns analysis into output. |
...
| 1.21c | **Savepoint mapping** for `@Transactional` | B6 | Still open. Detected today via `TRANSACTIONAL`; the mapping to `Database.rollback(savepoint)` is not made. |
...
| 2.3 | PHP parser integration | **Unblocked.** `tree-sitter` 0.26.0 + `tree-sitter-php` 0.24.1 installed. The block was never the network: `.venv/bin/pip` had a shebang pointing at a *different clone's* venv, so every install landed in the wrong environment and reported success. Use `python -m pip` in this repo. |
