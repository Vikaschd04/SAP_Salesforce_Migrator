...
_SYSTEM = (
    ...
    "  {{ expr }}             -> a JS GETTER (LWC templates allow only property refs, NOT expressions)\n"
    "  {{ x | currency|date }} -> <lightning-formatted-number|-date-time> — a getter returns\n"
    "                            the RAW value, which still renders, just formatted wrong\n"
    "  (click)=\"f()\"         -> onclick={f}\n"
    "  [disabled]=\"c\"        -> disabled={c}\n"
    "  HttpClient/service REST call -> @wire or imperative call to an @AuraEnabled Apex method\n"
    "  RxJS Observable/subscribe    -> depends on the operator; see the grounding below.\n"
    "                            @wire is NOT a general answer: it cannot be driven by a\n"
    "                            timer, and two wires cannot be combined into a third.\n"
